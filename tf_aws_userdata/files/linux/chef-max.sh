...
mkdir -p /etc/chef
s3 cp --region $${region} s3://$${S3_BUCKET}/utils/merge.sh  /etc/chef/merge.sh
s3 cp --region $${region} s3://$${S3_BUCKET}/utils/chef-max.sh /etc/chef/chef-max.sh
s3 cp --region $${region} s3://$${S3_BUCKET}/cookbooks/$${COOKBOOK}/$${COOKBOOK}.$${VERSION}.tar.gz /etc/chef/cookbooks.tar.gz
